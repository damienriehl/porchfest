...
- **Approach:**
  ...
  2. Make **one** versioned JSON Schema the canonical artifact — two independently owned copies are two authorities that can each drift with their own implementation while both suites stay green. The producer owns `venues-map.v1.schema.json` at `schema_version` 1.1.0 and its `.sha256` pin; the consumer carries byte-identical copies, and each side asserts the digest before validating data. [Amended 2026-08-29: the platform copy is v1.2.0 with generated_from widened to an enum (ask porchfest-2026-08-24-0045-u9-geocoder-and-map-schema, q2); the producer and site must catch up per packages/map/schemas/README.md before cutover.] Contract changes begin in the producer and update the schema and pin together; the site-cutover gate re-checks compatibility.
