...
- The Goal-1 producer normally owns the contract at
  `porchfest/schemas/venues-map.v1.schema.json` and its `.sha256` companion.
  Contract changes normally begin there. The v1.3.1 empty-venues relaxation
  originated in the platform publication work and must be propagated back to
  the producer and forward to the site before cutover.
...
The v1.3.0 contract made deployment-specific event, coordinate, schedule, and
...
surrounding whitespace. Version 1.3.1 allows an empty top-level `venues` array
while each published venue's `acts` array remains non-empty. This compatible
relaxation has its own version and digest so consumers can distinguish it from
the original v1.3.0 bytes.

Catch-up versions:

- 1.3.0 — deployment-neutral event, coordinate, schedule, and slot fields.
- 1.3.1 — empty `venues` allowed.

1. Update the Goal-1 producer at `porchfest/tools/render.py` to emit
   `schema_version: "1.3.1"`. It may keep emitting its season, date, time, city,
...
The v1.3.1 validator, deployment-neutral contract, and code-enforced minimum
