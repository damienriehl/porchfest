...
const scriptPath = path.join(import.meta.dirname, '..', 'assets', 'porchfest-map.js');
    ...
    map(element, options) {
      const map = {
        ...
        off(type, listener) {
          ...
          this.onceListeners[type] = (this.onceListeners[type] || []).filter((candidate) => (
            candidate !== listener
          ));
          return this;
        },
        fire(type) {
          (this.listeners[type] || []).slice().forEach((listener) => {
            if ((this.onceListeners[type] || []).includes(listener)) this.off(type, listener);
            listener.call(this);
          });
          return this;
        },
...
function response(payload, options = {}) {
  return {
    ok: options.ok !== false,
    json() {
      return options.jsonError ? Promise.reject(options.jsonError) : Promise.resolve(payload);
    }
  };
}
...
async function runScript(options = {}) {
  ...
  const window = {
    ...
    L: Object.prototype.hasOwnProperty.call(options, 'L') ? options.L : leaflet.L,
    AbortController: Object.prototype.hasOwnProperty.call(options, 'AbortController')
    ...
    dispatchEvent(event) {
      (windowListeners[event.type] || []).forEach((listener) => listener.call(window, event));
    },
...
function assertFailure(nodes) {
  ...
  assert.equal(nodes.mount.querySelectorAll('.porchfest-map-controls').length, 0);
  assert.equal(nodes.mount.querySelectorAll('.porchfest-genre-facet').length, 0);
}
...
function assertViewClasses(node, expected) {
  ...
  assert.equal(node.classList.contains('is-collapsed'), expected === 'collapsed');
}
...
function mixSrgb(from, to, toWeight) {
  ...
  return fromChannels.map((channel, index) => (
    channel * (1 - toWeight) + toChannels[index] * toWeight
  ));
}
...
test('collapsed cards and dimmed markers never fade their subtrees with opacity or filters', () => {
  const stylesheetRules = Array.from(stylesheetSource.matchAll(/([^{}]+)\{([^{}]*)\}/g));
  const cardRules = stylesheetRules.filter((rule) => rule[1].includes('.porchfest-venue-card'));

  assert.ok(cardRules.some((rule) => rule[1].includes('.porchfest-venue-card.is-collapsed')));
...
test('the stylesheet keeps one organizer-tunable fade factor for the pin and collapsed band', () => {
  const definitions = Array.from(stylesheetSource.matchAll(/--porchfest-filter-fade\s*:\s*([^;]+);/g));
  const fadeWeight = parseFloat(definitions[0] && definitions[0][1]) / 100;
  const fadedPin = mixSrgb(cssToken('--color-accent'), cssToken('--color-bg'), fadeWeight);
  ...
  assert.match(stylesheetSource, /\.porchfest-venue-card\.is-collapsed \.porchfest-venue-band\s*\{[^}]*var\(--porchfest-filter-fade\)/s);
  assert.match(stylesheetSource, /\.porchfest-marker-shell\.is-dimmed \.porchfest-marker-pin\s*\{[^}]*color-mix\([^}]*var\(--porchfest-filter-fade\)/s);
});

test('collapsed card CSS keeps an auto-height band above a smaller clipped performer peek', () => {
  const cardRule = stylesheetSource.match(/\.porchfest-venue-card\s*\{([^}]*)\}/);
  const collapsedRule = stylesheetSource.match(/\.porchfest-venue-card\.is-collapsed\s*\{([^}]*)\}/);
  const expandedActsRule = stylesheetSource.match(/\.porchfest-venue-card \.porchfest-venue-acts\s*\{([^}]*)\}/);
  const actsRule = stylesheetSource.match(/\.porchfest-venue-card\.is-collapsed \.porchfest-venue-acts\s*\{([^}]*)\}/);

  assert.ok(cardRule);
  assert.ok(collapsedRule);
  assert.ok(expandedActsRule);
  assert.ok(actsRule);
  assert.ok(parseFloat(collapsedRule[1].match(/min-height:\s*([\d.]+)px/)[1]) >= 44);
  assert.match(cardRule[1], /overflow:\s*hidden/);
  assert.doesNotMatch(collapsedRule[1], /max-height:/);
  const expandedMaxHeight = parseFloat(expandedActsRule[1].match(/max-height:\s*([\d.]+)rem/)[1]) * 16;
  const collapsedMaxHeight = parseFloat(actsRule[1].match(/max-height:\s*([\d.]+)px/)[1]);
  ...
  assert.match(actsRule[1], /(?:-webkit-)?mask-image:\s*linear-gradient\(\s*to bottom,/s);
...
test('collapsed card CSS keeps the map control at least 44px in both dimensions', () => {
  ...
  assert.ok(parseFloat(controlRule[1].match(/min-width:\s*([\d.]+)px/)[1]) >= 44);
...
test('collapsed card CSS expands for focus and disables motion when requested', () => {
  assert.match(stylesheetSource, /\.porchfest-venue-card\.is-collapsed:focus-within \.porchfest-venue-acts\s*\{[^}]*max-height:\s*100rem[^}]*mask-image:\s*none/s);
...
test('matching cards and marker pins receive distinct accent treatments', () => {
  assert.match(stylesheetSource, /\.porchfest-venue-card\.is-match\s*\{[^}]*border-color:\s*var\(--color-accent-ink\)[^}]*box-shadow:\s*none/s);
  assert.match(stylesheetSource, /\.porchfest-marker-shell\.is-match \.porchfest-marker-pin\s*\{[^}]*transform:[^;}]*scale\(/s);
});

test('neutral venue bands use the existing dark-ground and light-text tokens', () => {
  assert.match(stylesheetSource, /\.porchfest-venue-band\s*\{[^}]*background:\s*var\(--color-bg-dark\)[^}]*color:\s*var\(--color-text-light\)/s);
  assert.match(stylesheetSource, /\.porchfest-venue-band \.porchfest-venue-title\s*\{[^}]*color:\s*var\(--color-heading-light\)/s);
  assert.ok(contrastRatio(cssToken('--color-bg-dark'), cssToken('--color-text-light')) >= 4.5);
});
...
test('a collapsed venue band lightens past midpoint and flips to the dark text token at AA contrast', () => {
  ...
  const fadedBand = mixSrgb(cssToken('--color-bg-dark'), cssToken('--color-bg'), fadeWeight);
...
test('the matched band replaces the now-redundant two-tone accent rail', () => {
  assert.match(stylesheetSource, /\.porchfest-venue-card\.is-match\s*\{[^}]*box-shadow:\s*none/s);
  assert.match(stylesheetSource, /\.porchfest-venue-card\.is-match \.porchfest-venue-band\s*\{[^}]*var\(--color-accent-ink\)/s);
});

test('the act region retains a light ground and dark text below the venue band', () => {
  assert.match(stylesheetSource, /\.porchfest-venue-acts\s*\{[^}]*background:\s*#fff[^}]*color:\s*var\(--color-text\)/s);
});

test('Leaflet popup spacing lets the shared venue band reach the wrapper edges', () => {
  assert.match(stylesheetSource, /\.leaflet-popup-content-wrapper\s*\{[^}]*padding:\s*0[^}]*overflow:\s*hidden/s);
  assert.match(stylesheetSource, /\.leaflet-popup-content\s*\{[^}]*margin:\s*0/s);
  assert.match(stylesheetSource, /\.porchfest-map-popup\s*\{[^}]*padding:\s*0/s);
});

test('source never uses innerHTML', () => {
  assert.doesNotMatch(scriptSource, /\.innerHTML\b|\[\s*['"]innerHTML['"]\s*\]/);
});
...
test('a sorted card flies to and opens its venue-keyed marker', async () => {
  ...
  assert.equal(firstSortedCard.dataset.venueKey, run.testApi.venueKey(venues[1]));
...
test('a card opens its popup only from the flight move-completion callback', async () => {
  const selectedVenue = venue({ title: 'Flight Stage', lat: 44.99, lng: -93.18 });
  const run = await runScript({
...
test('a collapsed venue card still navigates to its marker without changing filter state', async () => {
  ...
  const rockCard = run.nodes.list.querySelectorAll('.porchfest-venue-card').find((card) => (
    card.dataset.venueKey === run.testApi.venueKey(venues[1])
  ));

  applyFilters(run, venues, '6-7', 'Folk');
  assertViewClasses(rockCard, 'collapsed');

  rockCard.querySelector('button.porchfest-show-on-map').dispatchEvent({ type: 'click' });
...
test('renders a permanent one-line hour control with exactly one active button', async () => {
  ...
  assert.equal(run.nodes.fullbleed.children.indexOf(controls) < run.nodes.fullbleed.children.indexOf(run.nodes.mapElement), true);
  assert.deepEqual(buttons.map((button) => button.textContent), ['All', '6–7 pm', '7–8 pm']);
...
test('hour buttons are accessible native buttons that update state and reapply the view', async () => {
  const venues = [
    ...
    venue({ title: 'Late', lat: 44.99, acts: [{ slot: '7-8', name: 'Late Act' }] })
  ];
  const run = await runScript({
    fetch: () => Promise.resolve(response({ venues }))
  });
  const buttons = run.nodes.fullbleed.querySelector('.porchfest-hour-control').querySelectorAll('button');
...
test('hour chips meet the 44px target and the toolbar is constrained to one line', () => {
  assert.match(stylesheetSource, /\.porchfest-filter-chip\s*\{[^}]*min-width:\s*44px[^}]*min-height:\s*44px/s);
  assert.match(stylesheetSource, /\.porchfest-map-toolbar\s*\{[^}]*flex-wrap:\s*nowrap/s);
  assert.match(stylesheetSource, /\.porchfest-hour-control\s*\{[^}]*flex-wrap:\s*nowrap/s);
  assert.match(stylesheetSource, /\.porchfest-filter-chip:focus-visible\s*\{[^}]*outline:/s);
});

test('filter chips use content width without shrinking below their touch target', () => {
  const chipRule = stylesheetSource.match(/\.porchfest-filter-chip\s*\{([^}]*)\}/);
...
test('derives genre chips from loaded data and orders them by frequency then alphabetically', async () => {
  const venues = [
    ...
    venue({
      title: 'Slash Stage',
      lat: 45,
      acts: [{ slot: '6-8', name: 'Indie Act', genre: 'Alternative / Indie Rock' }]
    })
  ];
  const run = await runScript({ fetch: () => Promise.resolve(response({ venues })) });
  const chips = run.nodes.fullbleed.querySelector('.porchfest-genre-chips').querySelectorAll('button');

  assert.deepEqual(
    chips.map((button) => button.textContent),
    ['All', 'Rock', 'Folk', 'Alternative / Indie Rock', 'Glitch Hop']
  );
  assert.equal(chips.filter((button) => button.textContent === 'Glitch Hop').length, 1);
  assert.equal(chips.filter((button) => button.textContent === 'Alternative / Indie Rock').length, 1);
});

test('genre disclosure uses the existing details pattern and All clears the genre facet', async () => {
  const venues = [
    venue({ title: 'Folk Stage', acts: [{ slot: '6-8', name: 'Folk Act', genre: 'Folk' }] }),
    venue({ title: 'Rock Stage', lat: 44.99, acts: [{ slot: '6-8', name: 'Rock Act', genre: 'Rock' }] })
  ];
  const run = await runScript({ fetch: () => Promise.resolve(response({ venues })) });
  const facet = run.nodes.fullbleed.querySelector('.porchfest-genre-facet');
  const summary = facet.querySelector('summary');
  const chips = facet.querySelector('.porchfest-genre-chips').querySelectorAll('button');
  ...
  assert.equal(chips.filter((button) => button.getAttribute('aria-pressed') === 'true').length, 1);
  run.nodes.list.children.forEach((card) => assertViewClasses(card, 'neutral'));
});

test('genre chip row scrolls horizontally with a visible scrollbar cue', () => {
  assert.match(stylesheetSource, /\.porchfest-genre-chips\s*\{[^}]*overflow-x:\s*auto[^}]*scrollbar-color:/s);
  assert.match(stylesheetSource, /\.porchfest-genre-chips::-webkit-scrollbar\s*\{[^}]*height:/s);
  assert.match(stylesheetSource, /\.porchfest-genre-summary:focus-visible[^}]*outline:/s);
});
...
test('starts south-to-north and reverses the existing card nodes through one sort button', async () => {
  ...
  assert.deepEqual(
    cardsBefore.map((card) => card.querySelector('.porchfest-venue-title').textContent),
...
test('sorting preserves every venue match state and marker lookup by venue key', async () => {
  const venues = [
    venue({ title: 'Middle Stage', lat: 44.98, acts: [{ slot: '6-7', name: 'Middle Act' }] }),
    venue({ title: 'North Stage', lat: 45.01, acts: [{ slot: '7-8', name: 'North Act' }] }),
    venue({ title: 'South Stage', lat: 44.94, acts: [{ slot: '6-7', name: 'South Act' }] })
  ];
  const run = await runScript({ fetch: () => Promise.resolve(response({ venues })) });
  const hourButtons = run.nodes.fullbleed.querySelector('.porchfest-hour-control').querySelectorAll('button');
  const sortButton = run.nodes.fullbleed.querySelector('.porchfest-sort-button');
  const lookup = run.testApi.getMarkersByVenueKey();
  ...
  run.nodes.list.children.forEach((card) => {
    ...
    const venueForCard = venues.find((item) => run.testApi.venueKey(item) === card.dataset.venueKey);
    assert.equal(lookup[card.dataset.venueKey].coordinates[0], venueForCard.lat);
  });
...
test('filtering keeps non-matching venues in the lineup and their markers on the map', async () => {
  const venues = [
    ...
    venue({ title: 'Late Stage', lat: 44.99, lng: -93.18, acts: [{ slot: '7-8', name: 'Late Act' }] })
  ];
  const run = await runScript({ fetch: () => Promise.resolve(response({ venues })) });

  applyFilters(run, venues, '6-7', 'all');

  assert.equal(run.nodes.list.querySelectorAll('.porchfest-venue-card').length, 2);
...
test('hour filters resolve every slot value and a two-act venue', async () => {
  const venues = [
    venue({ title: 'Early', acts: [{ slot: '6-7', name: 'Early Act' }] }),
    venue({ title: 'Late', lat: 44.981, acts: [{ slot: '7-8', name: 'Late Act' }] }),
    venue({ title: 'Full', lat: 44.982, acts: [{ slot: '6-8', name: 'Full Act' }] }),
    venue({
      title: 'Two Act',
      lat: 44.983,
      acts: [
  ...
  applyFilters(run, venues, '6-7', 'all');
  assert.deepEqual(cards.map((card) => card.classList.contains('is-match')), [true, false, true, true]);
  assertViewClasses(cards[1], 'collapsed');

  applyFilters(run, venues, '7-8', 'all');
  assert.deepEqual(cards.map((card) => card.classList.contains('is-match')), [false, true, true, true]);
...
test('facets combine with AND when hour matches but genre does not', async () => {
  const venues = [
    venue({ title: 'Rock Stage', acts: [{ slot: '6-7', name: 'Rock Act', genre: 'Rock' }] }),
    venue({
      title: 'Folk Stage',
      lat: 44.99,
      lng: -93.18,
...
test('filtering preserves sorted card and marker pairing by venue key', async () => {
  const venues = [
    venue({ title: 'Early Folk', acts: [{ slot: '6-7', name: 'Folk Act', genre: 'Folk' }] }),
    venue({
      title: 'Late Rock',
      lat: 44.99,
      lng: -93.18,
...
test('genre tags split on commas but preserve slash compounds', async () => {
  ...
  assert.deepEqual(
    Array.from(run.testApi.genreTags({ genre: 'Alternative / Indie Rock, Folk' })),
    ['Alternative / Indie Rock', 'Folk']
  );
  assert.equal(run.testApi.genreTags({ genre: 'Alternative / Indie Rock' }).length, 1);
});
...
test('returning both facets to All removes every view class', async () => {
  ...
  run.nodes.list.children.forEach((card) => assertViewClasses(card, 'neutral'));
  run.leaflet.records.markers.forEach((marker) => assertViewClasses(marker.element, 'neutral'));
});

test('a zero-match combination explains how to return to All without removing venues', async () => {
  const venues = [venue({ acts: [{ slot: '6-7', name: 'Folk Act', genre: 'Folk' }] })];
  const run = await runScript({
    fetch: () => Promise.resolve(response({ venues }))
  });
  ...
  assert.equal(
    ...
    run.nodes.fullbleed.children.indexOf(run.nodes.fullbleed.querySelector('.porchfest-map-controls')) + 1
  );
  assert.equal(
...
test('shows the fallback when the request rejects', async () => {
  const { nodes } = await runScript({ fetch: () => Promise.reject(new Error('offline')) });
  assertFailure(nodes);
});
...
test('shows the fallback when JSON parsing fails', async () => {
  const { nodes } = await runScript({
    fetch: () => Promise.resolve(response(null, { jsonError: new SyntaxError('bad JSON') }))
  });
  assertFailure(nodes);
});
...
test('shows the empty state when no venue has an act', async () => {
  ...
  assert.match(nodes.status.textContent, /lineup is not on the interactive map yet/i);
  assert.equal(nodes.mapElement.hidden, true);
  assert.equal(nodes.listSection.hidden, true);
  assert.equal(nodes.mount.querySelectorAll('.porchfest-map-controls').length, 0);
  assert.equal(nodes.mount.querySelectorAll('.porchfest-genre-facet').length, 0);
...
test('shows the fallback and no partial list for invalid coordinates', async () => {
  const { nodes, leaflet } = await runScript({
    fetch: () => Promise.resolve(response({ venues: [venue({ lat: '44.98' })] }))
  });
  assertFailure(nodes);
...
test('renders one marker and one semantic list item per venue', async () => {
  const venues = [
    venue(),
    venue({ title: 'Oak Stage', address: '200 Festival Ave', lat: 44.99, lng: -93.18 })
  ];
  ...
  assert.deepEqual(
    ...
    [[44.98, -93.19], [44.99, -93.18]]
  );
  assert.equal(leaflet.records.tileLayers[0].options.maxZoom, 19);
  assert.match(leaflet.records.tileLayers[0].options.attribution, /OpenStreetMap/);
...
test('renders an accessible 44px marker with a decorative SVG musical note', async () => {
  ...
  assert.equal(notePath.getAttribute('d'), 'M12 3v10.55A4 4 0 1 0 14 17V7h4V3h-6z');
  assert.equal(marker.element.getAttribute('role'), 'button');
  assert.equal(marker.element.getAttribute('aria-label'), 'Garden Stage: show lineup');
  assert.equal(marker.element.tabIndex, 0);
  assert.match(stylesheetSource, /\.porchfest-marker-shell\s*\{[^}]*width:\s*44px[^}]*height:\s*44px/s);
  assert.match(stylesheetSource, /\.porchfest-marker-shell:focus-visible\s*\{[^}]*outline:/s);
  assert.match(stylesheetSource, /\.porchfest-marker-pin\s*\{[^}]*border:\s*3px solid #fff[^}]*box-shadow:/s);
...
test('renders a 6-8 act and preserves the producer order for two-act venues', async () => {
  ...
  assert.deepEqual(
    cards[0].querySelectorAll('.porchfest-act-slot').map((node) => node.textContent),
    ['6–8 pm']
  );
  assert.deepEqual(
    cards[1].querySelectorAll('.porchfest-act-slot').map((node) => node.textContent),
    ['6–7 pm', '7–8 pm']
  );
});
...
test('omits a whitespace-equivalent address in both the list and popup', async () => {
  ...
  assert.equal(nodes.list.querySelectorAll('.porchfest-venue-address').length, 0);
...
test('keeps a meaningfully different address in both the list and popup', async () => {
  ...
  assert.equal(nodes.list.querySelector('.porchfest-venue-address').textContent, '2357 Bayless Place');
  assert.equal(popup.querySelector('.porchfest-venue-address').textContent, '2357 Bayless Place');
});
...
test('uses hostnames for visible link text and keeps link types in accessible labels', async () => {
  const linkedVenue = venue({
    acts: [{
      ...
      links: [
        ...
        { label: 'Website', url: 'https://www.instagram.com/grovekeepermusic' }
      ]
    }]
  });
  const { nodes } = await runScript({
  ...
  const anchors = nodes.list.querySelectorAll('.porchfest-act-links')[0].querySelectorAll('a');

  assert.deepEqual(anchors.map((anchor) => anchor.textContent), ['open.spotify.com', 'instagram.com']);
  assert.equal(anchors[0].getAttribute('aria-label'), 'Listen — open.spotify.com');
  assert.equal(anchors[0].getAttribute('title'), 'Listen — open.spotify.com');
  assert.equal(anchors[1].getAttribute('aria-label'), 'Website — instagram.com');
...
test('popup CSS caps content and wraps long unbroken text', () => {
  assert.match(stylesheetSource, /\.leaflet-popup-content\s*\{[^}]*max-width:[^;}]*100vw/s);
  assert.match(stylesheetSource, /\.leaflet-popup-content\s*\{[^}]*overflow-wrap:\s*anywhere/s);
  assert.match(stylesheetSource, /\.porchfest-act-links a[^}]*overflow-wrap:\s*anywhere/s);
  assert.match(stylesheetSource, /\.leaflet-popup-content\s*\{[^}]*overflow-x:\s*hidden/s);
});
...
test('renders hostile text literally and drops unsafe links in list and popup', async () => {
  const hostile = venue({
    ...
    acts: [{
      ...
      links: [
        { label: '<script>alert(4)</script>', url: 'https://example.com/safe?q=<tag>' },
  ...
  [listAnchors[0], popupAnchors[0]].forEach((anchor) => {
    ...
    assert.equal(anchor.getAttribute('aria-label'), '<script>alert(4)</script> — example.com');
