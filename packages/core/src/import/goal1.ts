...
export const goal1ArtifactFiles = Object.freeze([
...
export function importGoal1Season(
  ...
  const artifacts = readArtifacts(options.artifactsDirectory);
...
function applySupersessionGroup(
  ...
  for (const [sourceKey, rawEntry] of Object.entries(group)) {
    ...
    if (!alreadySuperseded) {
      if (recordType === "venue") {
        state.core.seasons.supersedeVenue(
          source.id,
          source.version,
          canonical.id,
        );
      } else {
        state.core.seasons.supersedeAct(
          source.id,
          source.version,
          canonical.id,
        );
    ...
    if (
      ...
      state.core.seasons.supersedeContact(
        currentContact.id,
  ...
  for (const rawVenue of array(matches.venues)) {
    ...
    for (const [index, definition] of GOAL1_SLOT_DEFINITIONS.entries()) {
      ...
      if (heldVirtualKey) {
        ...
        const decideBy = new Date(`${decideByText}T00:00:00.000Z`);
...
function annotateUnmatchedAndFloating(
  ...
  for (const [index, raw] of array(matches.unmatched_venues).entries()) {
    ...
    for (const [field, label] of [
      ...
      if (value) {
        addAnnotation(
          ...
          "venue",
          venue.id,
          `unmatched:${index}:${field}`,
          `${label}: ${value}`,
        );
      }
    }
  }
  for (const [index, raw] of array(matches.floating_performers).entries()) {
    ...
    for (const [noteIndex, note] of stringList(entry.email_notes).entries()) {
      addAnnotation(
        ...
        "act",
        act.id,
        `floating:${index}:email-note:${noteIndex}`,
        `Email note: ${note}`,
      );
    }
  }
...
function importCoordinates(state: ImportState, geocache: JsonObject): void {
  ...
  for (const [address, rawCoordinate] of Object.entries(geocache)) {
    ...
    const status = found === null ? "created" : "found";
    if (found === null) {
      bindImportKey(state, {
    ...
    } else if (found.recordId !== imported.id) {
      throw new Error(
        `Coordinate import key ${naturalKey} no longer resolves to venue ${venue.id}.`,
      );
    }
    increment(state.report, "coordinate", status);
    state.report.geocache.hits.push({
      address,
      venueId: venue.id,
      coordinateId: imported.id,
      status,
      reviewStatus: imported.status,
      rejectionCode: imported.rejectionCode,
    });
...
function mapOverrideField(
  ...
  if (field in directActFields) {
    ...
    actChanges.notes = performerNotes(row, extracted.residue);
  }
}
...
function readArtifacts(directory: string): Goal1Artifacts {
  const submissions = readJson(join(directory, goal1ArtifactFiles[0]!));
  const matches = readJson(join(directory, goal1ArtifactFiles[1]!));
  const geocache = readJson(join(directory, goal1ArtifactFiles[2]!));
  return {
...
function eventDateValue(event: JsonObject): string {
  ...
  if (Number.isNaN(parsed.getTime())) {
    throw new Error(`Event date_display is not parseable: ${display}`);
  }
...
function positiveContent(value: unknown): boolean {
  ...
  return text !== null && !placeholder(text) && !/^(?:no|false)\b/i.test(text);
}
