...
export function generateSeasonFixture(outputDirectory = here): void {
  ...
  writeJson(join(outputDirectory, "out", "submissions.json"), {
    hosts,
    performers,
  });
  writeJson(join(outputDirectory, "private", "matches-2026.json"), matches);
  writeJson(join(outputDirectory, "private", "geocache.json"), geocache);
  const toolsDirectory = join(outputDirectory, "tools");
  mkdirSync(toolsDirectory, { recursive: true });
  writeFileSync(
    join(toolsDirectory, "geocode.py"),
    [
      "# Synthetic fixture bounds; this file is not an executable geocoder.",
